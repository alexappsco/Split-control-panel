...
// Client-side hook with translation support
export function useFormat() {
  const t = useTranslations();
  ...
  const formatDate = (date: InputValue, newFormat?: string) => {
    ...
    return date
      ? format(new Date(date), fm).replace(
          /January|February|March|April|May|June|July|August|September|October|November|December|Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec|AM|PM/g,
          (matched) => t(`Global.Date.${matched.toLowerCase().slice(0, 3)}`)
        )
      : '-';
  };
  ...
  const formatTime = (date: InputValue, newFormat?: string) => {
    ...
    return date
      ? format(new Date(date), fm).replace(/AM|PM/g, (matched) =>
          t(`Global.Date.${matched.toLowerCase()}`)
        )
      : '-';
  };
...
// Server-side function with translation support
export async function getFormate() {
  const t = await getTranslations();
